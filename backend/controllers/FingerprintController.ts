...
export const enrollFingerprint = (): Promise<number | null> => {
    return fetch(`https://${process.env.ESP_LINK}/enroll`, {
        method: "POST",
...
export const verifyFingerprint = (): Promise<number | null> => {
    return fetch(`https://${process.env.ESP_LINK}/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    })
    .then(response => response.json())
...
export const deleteFingerprint = (fingerId: number): Promise<boolean> => {
    return fetch(`https://${process.env.ESP_LINK}/clearOne?id=${fingerId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
